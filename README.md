<p align="center">
  <img src="https://raw.githubusercontent.com/TWRAR/Engine/main/assets/logo.png" width="300" alt="TWRAR — The Website Recorder And Replayer">
</p>

# TWRAR .github

GitHub organisation profile for [TWRAR](https://github.com/TWRAR).

Website: https://twrar.stuxie.dev  
Repository: https://github.com/TWRAR/.github

## Contents

- **`profile/README.md`** — the public-facing organisation profile shown on
  the [TWRAR organisation page](https://github.com/TWRAR) on GitHub.

## Repositories

| Repo | Description |
|------|-------------|
| [Engine](https://github.com/TWRAR/Engine) | The recorder/replayer engine and GUI |
| [Website](https://github.com/TWRAR/Website) | Source for twrar.stuxie.dev |

---

*Written & Maintained by <img src="https://github.com/StuxieDev.png" height="14" alt="StuxieDev" valign="middle"> [StuxieDev](https://stuxie.dev).*

*[A StuxieDev Project](https://projects.stuxie.dev)*
